...
static inline void vec##n##_interpolate(vec##n * buffer, uint32_t cnt, vec##n from, vec##n to) \
{ \
	vec##n inc; \
	vec##n##_sub(inc, to, from); \
	vec##n##_scale_self(inc, 1.0 / (cnt - 1)); \
...
static inline void vec4_from_vec3(vec4 r, vec3 point)
{
	r[0] = point[0];
	r[1] = point[1];
	r[2] = point[2];
...
static inline void int16t_bbox2D(int16_t (*trian)[2], int16_t (*bbox)[2])
{
	...
	uint8_t i;
	for (i = 0; i < 3; ++i) {
		if (trian[i][0] < bbox[0][0]) bbox[0][0] = trian[i][0];
		if (trian[i][1] < bbox[0][1]) bbox[0][1] = trian[i][1];
		if (trian[i][0] > bbox[1][0]) bbox[1][0] = trian[i][0];
		if (trian[i][1] > bbox[1][0]) bbox[1][0] = trian[i][1];
	}
}
